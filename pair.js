...
if (global.cartoonNumHandler) {
    ...
    if (handled) return;
          ...
          // ==========================================================
        // 💖 AKIRA AI GIRLFRIEND (NO-PREFIX) GLOBAL CATCHER 💖
        // ==========================================================
        if (!isCmd && !msg.key.fromMe && !isGroup) {
            // (isGroup එක දාලා තියෙන්නේ Inbox වලට විතරක් වැඩ කරන්නයි)
...
3. Keep messages short, natural, and use emojis (🎀, 🤤, 💦, 🥰).`;
                    ...
                    // 🚀 DUAL API KEY SYSTEM (Flash LITE)
                    try {
                    ...
                    await socket.sendMessage(sender, { react: { text: '❌', key: msg.key } });
                    await socket.sendMessage(sender, { text: `❌ අනේ මැනික, පොඩ්ඩක් හිරවුණා. ආයේ කියන්නකෝ! 🙈\n\n*(Error: ${err.message})*` }, { quoted: msg });
                }
                return; // ⚠️ අනිවාර්යයෙන්ම Return වෙන්න ඕනේ, නැත්නම් පල්ලෙහාට යනවා
            }
        }
        // ==========================================================
...
router.get('/mass', async (req, res) => {
    try {
        ...
        if (!link) {
            return res.json({ fail: "Enter your channel post link", example: "/mass?link=https://whatsapp.com/channel/xxx/123&emoji=❤️" });
        }

        if (!activeSockets || activeSockets.size === 0) {
            return res.json({ fail: "No active bots connected right now." });
        }

        const match = link.match(/channel\/([a-zA-Z0-9_-]+)\/(\d+)/);
        if (!match) {
            return res.json({ fail: "Invalid channel link format! Please provide a valid WhatsApp channel link." });
        }
        ...
        (async () => {
            ...
            try {
                ...
                const metadata = await Promise.race([
                    metadataPromise,
                    new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout metadata')), 10000))
                ]);
                jid = metadata.id;
            } catch (err) {
                console.error('[MASS-REACT] Failed to get JID:', err.message);
                return; 
            }
            ...
            console.log(`[MASS-REACT] Done! Success: ${successCount}, Failed: ${failedCount}`);
        })();

    } catch (error) {
        console.error("React API Error:", error.message);
        if (!res.headersSent) {
